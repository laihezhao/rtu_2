...
int local_di(int addr)
{
	...
	if (r < 0) {
		fprintf(stderr, "read di devices failed!\n");
		return -1;
	}
...
float local_ai(int addr)
{
	...
	if (nchannel < 1 || nchannel > 4) {
		fprintf(stderr, "input parameter erroe!\n");
		return -9.99;
	}
...
int open_di_device(void)
{
	...
	if (difd < 0) {
		fprintf(stderr, "open /dev/DI device failed!\n");
		return -1;
	}
...
int open_do_device(void)
{
	...
	if (dofd < 0) {
		fprintf(stderr, "open /dev/switch device failed!\n");
		return -1;
	}
...
int open_ai_device(void)
{
	...
	if (aifd < 0) {
		fprintf(stderr, "open /dev/AD2543 device failed!\n");
		return -1;
	}
