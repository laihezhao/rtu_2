...
static void timer_thread_sec(union sigval v)
{
	...
	if (r != 0) {
		fprintf(stderr, "error at %s-%d: get system time failed\n", __FUNCTION__, __LINE__);
		return ;
	}
...
//
					...
					snprintf(qstr, sizeof(qstr), "delete from minutedata where "
						"(julianday(datetime('now','localtime')) - "
						"julianday(datetime(datatime)))*24*60*60>60*60*5 and polid='%s';",psitepols->polid);
...
static void unlink_mqueue(void)
{
	...
	for (i = 1; i < (servernum + 1); i++) {
		if (mq_unlink(client_mqfile[i]) == 0)
			fprintf(stdout, "Message queue %s removed from system.\n", client_mqfile[i]);

		if (mq_unlink(mqfile[i]) == 0)
			fprintf(stdout, "Message queue %s removed from system.\n", mqfile[i]);
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (r != 0) {
		fprintf(stderr, "create watchdog timer failed\n");
		return 2;
	}

	r = start_watchdog_timer();
	if (r != 0) {
		fprintf(stderr, "start watchdog timer failed\n");
		return 2;
	}
	...
	if (r != 0) {
	       fprintf(stderr, "create timer routine failed: %s\n", strerror(errno));
		return 3;
	}
